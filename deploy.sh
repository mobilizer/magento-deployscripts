...
function usage {
    ...
    echo "Usage:"
    echo ""
    echo "$0 -r <packageUrl> -t <targetDir> -e <environment> [-u <downloadUsername>] [-p <downloadPassword>] [-a <awsCliProfile>] [-i <systemStoragePath>] [-d] [-c]"
    echo ""
...
while getopts 'r:t:u:p:e:a:i:d:c' OPTION ; do
case "${OPTION}" in
        r) PACKAGEURL="${OPTARG}";;
...
${RELEASEFOLDER}/tools/install.sh -e "${ENVIRONMENT}" -i "${SYSTEMSTORAGEPATH}" || { echo "Installing package failed"; exit 1; }
